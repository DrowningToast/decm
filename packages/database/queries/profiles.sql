...
INSERT INTO profiles (
    ...
    CASE 
        ...
        THEN pgp_sym_decrypt(profile_picture_url::bytea, sqlc.arg(encryption_key))
        ELSE NULL 
    END::text as profile_picture_url,
    is_first_name_public,
    CASE 
        WHEN first_name IS NOT NULL 
        THEN pgp_sym_decrypt(first_name::bytea, sqlc.arg(encryption_key))
        ELSE NULL 
    END::text as first_name,
    is_last_name_public,
    CASE 
        WHEN last_name IS NOT NULL 
        THEN pgp_sym_decrypt(last_name::bytea, sqlc.arg(encryption_key))
        ELSE NULL 
    END::text as last_name,
    is_email_public,
    CASE 
        WHEN email IS NOT NULL 
        THEN pgp_sym_decrypt(email::bytea, sqlc.arg(encryption_key))
        ELSE NULL 
    END::text as email,
    is_bio_public,
    CASE 
        WHEN bio IS NOT NULL 
        THEN pgp_sym_decrypt(bio::bytea, sqlc.arg(encryption_key))
        ELSE NULL 
    END::text as bio,
    is_phone_number_public,
    CASE 
        WHEN phone_number IS NOT NULL 
        THEN pgp_sym_decrypt(phone_number::bytea, sqlc.arg(encryption_key))
        ELSE NULL 
    END::text as phone_number,
    is_address_public,
    CASE 
        WHEN address IS NOT NULL 
        THEN pgp_sym_decrypt(address::bytea, sqlc.arg(encryption_key))
        ELSE NULL 
    END::text as address,
    is_academic_institution_public,
    CASE 
        WHEN academic_institution IS NOT NULL 
        THEN pgp_sym_decrypt(academic_institution::bytea, sqlc.arg(encryption_key))
        ELSE NULL 
    END::text as academic_institution,
    is_academic_email_public,
    CASE 
        WHEN academic_email IS NOT NULL 
        THEN pgp_sym_decrypt(academic_email::bytea, sqlc.arg(encryption_key))
        ELSE NULL 
...
AND pgp_sym_decrypt(email::bytea, sqlc.arg(encryption_key)) = sqlc.arg(email_search);
