...
import type { EventRegistrationInvitationImportEventParticipantsRequest } from "@decm/api";
...
export function useImportParticipants(eventId: string) {
    ...
    const { mutateAsync: importParticipants, isPending: isImportingParticipants } = useMutation({
        ...
        mutationFn: (data: EventRegistrationInvitationImportEventParticipantsRequest) =>
