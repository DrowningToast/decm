...
func (u *OnboardUsecase) RegisterWithWalletAddress(ctx context.Context, signedMsg string) (*uuid.UUID, *string, error) {
	...
	slog.Info("No row found in authentication_credentials table (handled as Not Found, creating new credential)", "wallet_address", walletAddress.Hex())
...
func (u *OnboardUsecase) RegisterWithGoogle(ctx context.Context, token *oauth2.Token, password string) (*uuid.UUID, *string, error) {
	...
	slog.Info("No row found in authentication_credentials table (handled as Not Found, creating new credential)", "email", userInfo.Email)
	...
	slog.Info("No row found in profiles table (handled as Not Found, creating new profile)", "email", userInfo.Email)
...
func (u *OnboardUsecase) CheckOnboardStatusWithJwt(ctx context.Context, currentUser auth.JwtClaims) (*auth.JwtPayload, *uuid.UUID, error) {
	...
	if err != nil {
		...
		if errors.As(err, &notFoundErr) {
			slog.Info("No row found in profiles table (handled as Not Found)", "user_id", currentUser.UserId)
			return jwt, nil, nil
		}
...
func (u *OnboardUsecase) CheckOnboardStatusWithGoogleConnectorRef(ctx context.Context, accessToken string, expiresIn int) (*auth.JwtPayload, *uuid.UUID, error) {
	...
	if err != nil {
		var notFoundErr *customerror.Err
		if errors.As(err, &notFoundErr) {
			slog.Info("No row found in authentication_credentials table (handled as Not Found)", "email", userInfo.Email)
			return nil, nil, nil
		}
	...
	if err != nil {
		var notFoundErr *customerror.Err
		if errors.As(err, &notFoundErr) {
			slog.Info("No row found in profiles table (handled as Not Found)", "credential_id", credential.Id)
			return jwt, nil, nil
		}
		return jwt, nil, errors.Wrap(err, "failed to check for existing profile")
	}
...
func (u *OnboardUsecase) CheckOnboardStatusWithWalletAddress(ctx context.Context, messageSignature string) (*auth.JwtPayload, *uuid.UUID, error) {
	...
	if err != nil {
		...
		slog.Info("No row found in authentication_credentials table (handled as Not Found)", "wallet_address", walletAddress.Hex())
	}
	if credential == nil {
		return nil, nil, nil
	}
